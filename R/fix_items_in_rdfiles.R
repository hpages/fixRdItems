...
fix_items_in_lines <- function(lines, eol_marker="<<<-XXX->>>")
{
    ...
    if (any(grepl(eol_marker, lines, fixed=TRUE)))
        stop("input lines contain occurrences of 'eol_marker', ",
...
fix_items_in_file <- function(infile, outfile=infile, dry.run=FALSE,
                              sep="\n", eol_marker="<<<-XXX->>>")
{
    ...
    if (!is.logical(dry.run) || length(dry.run) != 1L || is.na(dry.run))
        stop("'dry.run' must be TRUE or FALSE")
...
fix_items_in_rdfiles <- function(indir=".", outdir=indir, dry.run=FALSE,
                                 pattern="\\.Rd$",
                                 sep="\n", eol_marker="<<<-XXX->>>")
{
    ...
    message(">> Parsing ", nfiles, " files to replace all ",
            ...
            ">>    \\item{\\code{...}:}{ ...}")
    if (dry.run)
        message("START DRY RUN (output files won't actually get written)")
    else
        message("START")
    ...
    for (i in seq_len(nfiles)) {
        ...
        nb_replacements <- fix_items_in_file(infile, outfile, dry.run=dry.run,
    ...
    if (dry.run)
        message("END DRY RUN (output files didn't actually get written)")
    else
        message("END")
    total_replacements
}
